...
cp "/home/anny-ribeiro/amazonQ/app/security_analyzer.sh.bak_add" "/home/anny-ribeiro/amazonQ/app/security_analyzer.sh"
echo "Script restaurado do backup mais recente."

# Vamos usar uma abordagem mais simples
# Restaurar o backup original
cp "/home/anny-ribeiro/amazonQ/app/security_analyzer.sh.bak" "/home/anny-ribeiro/amazonQ/app/security_analyzer.sh"
echo "Script restaurado do backup original."

# Criar um script separado para gerar relatórios
cat > "/home/anny-ribeiro/amazonQ/app/generate_report.sh" << 'EOF'
#!/bin/bash
...
chmod +x "/home/anny-ribeiro/amazonQ/app/generate_report.sh"

# Modificar a função analyze_file para usar o script de relatório
sed -i '/^analyze_file() {/,/^}/ s/log_message "Arquivo analisado: $filepath (MD5: $md5_hash, SHA256: $sha256_hash)"/log_message "Arquivo analisado: $filepath (MD5: $md5_hash, SHA256: $sha256_hash)"\n    \n    # Gerar e abrir relatório HTML\n    "$(dirname "$0")\/generate_report.sh" "Análise de Arquivo" "$filepath" "$LOG_FILE"/' "/home/anny-ribeiro/amazonQ/app/security_analyzer.sh"

# Modificar a função analyze_url para usar o script de relatório
sed -i '/^analyze_url() {/,/^}/ s/log_message "URL analisada: $url"/log_message "URL analisada: $url"\n    \n    # Gerar e abrir relatório HTML\n    "$(dirname "$0")\/generate_report.sh" "Análise de URL" "$url" "$LOG_FILE"/' "/home/anny-ribeiro/amazonQ/app/security_analyzer.sh"

# Modificar a função analyze_domain para usar o script de relatório
sed -i '/^analyze_domain() {/,/^}/ s/log_message "Domínio analisado: $domain"/log_message "Domínio analisado: $domain"\n    \n    # Gerar e abrir relatório HTML\n    "$(dirname "$0")\/generate_report.sh" "Análise de Domínio" "$domain" "$LOG_FILE"/' "/home/anny-ribeiro/amazonQ/app/security_analyzer.sh"

# Modificar a função analyze_hash para usar o script de relatório
sed -i '/^analyze_hash() {/,/^}/ s/log_message "Hash analisado: $hash ($hash_type)"/log_message "Hash analisado: $hash ($hash_type)"\n    \n    # Gerar e abrir relatório HTML\n    "$(dirname "$0")\/generate_report.sh" "Análise de Hash" "$hash" "$LOG_FILE"/' "/home/anny-ribeiro/amazonQ/app/security_analyzer.sh"

# Modificar a função analyze_email para usar o script de relatório
sed -i '/^analyze_email() {/,/^}/ s/log_message "Email analisado: $email"/log_message "Email analisado: $email"\n    \n    # Gerar e abrir relatório HTML\n    "$(dirname "$0")\/generate_report.sh" "Análise de Email" "$email" "$LOG_FILE"/' "/home/anny-ribeiro/amazonQ/app/security_analyzer.sh"

# Modificar a função analyze_email_header para usar o script de relatório
sed -i '/^analyze_email_header() {/,/^}/ s/log_message "Cabeçalho de email analisado: $header_file (Ameaças: $suspicious_count)"/log_message "Cabeçalho de email analisado: $header_file (Ameaças: $suspicious_count)"\n    \n    # Gerar e abrir relatório HTML\n    "$(dirname "$0")\/generate_report.sh" "Análise de Cabeçalho de Email" "$header_file" "$LOG_FILE"/' "/home/anny-ribeiro/amazonQ/app/security_analyzer.sh"
